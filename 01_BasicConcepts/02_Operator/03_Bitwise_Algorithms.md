...
- `-n` is called the 2's compliment. See end of page for more details on 2's compliment 
- For example, if `n = 5` `0000 0101` and you write `-n`, the result in binary will be `1111 1011` as described above, which represents -5 in two's complement form.
- **Example:**
  - If `n = 10` (binary `1010`), then `n & (-n)` gives `2` (binary `0010`).
  - If `n = 12` (binary `1010`), then `n & (-n)` gives `4` (binary `0010`).
  - If `n = 18` (binary `1010`), then `n & (-n)` gives `2` (binary `0010`).
